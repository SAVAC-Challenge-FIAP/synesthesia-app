...
**Nunca use `eas build`.** A cota está reservada para a publicação final. Só build local — setup
completo do toolchain em [build-e-deploy.md](./build-e-deploy.md).
...
destinatários do Mensagens, por exemplo). Essas imagens **não vão** para `docs/previews/` nem para
