# ADR-0002 — Geração do `.mp4` (imagem + trilha)

**Status**: Aceita e implementada · **Data**: 2026-07-09 · **Decisão final**: opção B, sobre **Media3 Transformer** (ver nota no fim)

> **Contexto original:** T-01 (Fase 1) implementada como *pacote composto*; o Sávio quer o **vídeo pronto** ao compartilhar. Este doc reúne o levantamento que fundamentou a **T-07**. Ver `specs/001-synesthesia-mvp/tasks.md` e `plan.md`.
...
**Ressalvas de B:**
- Exige sair do Expo Go → `npx expo run:android` (precisa de Android Studio/SDK) ou EAS Build. Isso muda o fluxo de testes do Sávio (hoje é QR code no Expo Go, travado no SDK 54 pela versão do app Expo Go no celular — ver `docs/_archive/STATUS-2026-08-04.md`).
...
## Nota de implementação (posterior a este levantamento)

A opção B foi implementada em [`modules/video-muxer`](../../modules/video-muxer) — mas sobre o
**Media3 Transformer** do Google (H.264 + AAC) em vez de `MediaCodec`/`MediaMuxer` manuais. Mesmo
raciocínio da opção B (API de sistema, sem binário, sem licença de codec), com menos código próprio
para manter as diferenças de encoder entre fabricantes. Ver [[video-muxer-usa-media3]] e
`docs/runbooks/` para o fluxo de build. iOS ficou como não implementado — o JOVI Challenge foi
validado só em Android.
