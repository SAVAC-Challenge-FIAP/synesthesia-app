...
- [ ] T020 [US3] Instrumentar com marcações de tempo as três etapas de `analyzePhotoAndSuggest` em `src/services/music.ts` (redução da imagem, chamada ao Gemini, resolução das faixas) e registrar os números em `baseline.md`
- [ ] T021 [US3] Atacar a etapa dominante identificada em T020 — hipóteses na ordem de [research.md](./research.md) R3: antecipar a redução da imagem para o instante da captura; reduzir o payload enviado; só então considerar troca de modelo
